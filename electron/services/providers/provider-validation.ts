...
function logValidationStatus(provider: string, status: number): void {
  logger.info(`[clawcorp-validate] ${provider} HTTP ${status}`);
}
...
function logValidationRequest(
  ...
  logger.info(
    `[clawcorp-validate] ${provider} request ${method} ${sanitizeValidationUrl(url)} headers=${JSON.stringify(sanitizeHeaders(headers))}`,
  );
}
...
async function validateOpenAiCompatibleKey(
  ...
  if (modelsResult.status === 404) {
    logger.info(
      `[clawcorp-validate] ${providerType} /models returned 404, falling back to ${apiProtocol} probe`,
    );
    if (apiProtocol === 'openai-responses') {
...
async function validateAnthropicHeaderKey(
  ...
  if (
    ...
    logger.info(
      `[clawcorp-validate] ${providerType} /models returned error, falling back to /messages probe`,
    );
