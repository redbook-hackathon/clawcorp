...
const CONSOLE_MIRROR_ENABLED = process.env.CLAWCORP_LOG_TO_CONSOLE === '1';
...
export function initLogger(): void {
  try {
    ...
    logFilePath = join(logDir, `clawcorp-${timestamp}.log`);

    // Write a separator for new session (sync is OK — happens once at startup)
    const sessionHeader = `\n${'='.repeat(80)}\n[${new Date().toISOString()}] === ClawCorp Session Start (v${app.getVersion()}) ===\n${'='.repeat(80)}\n`;
