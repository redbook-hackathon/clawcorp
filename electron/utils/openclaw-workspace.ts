...
export async function hireFromMarketplaceTemplate(
  ...
  // Handle existing workspace: reuse if valid, clean up if broken
  if (await fileExists(newWorkspace)) {
    const entries = await readdir(newWorkspace);
    const hasMarkdown = entries.some(e => e.endsWith('.md'));
    if (hasMarkdown) {
      // Workspace exists with content — agent was already hired, return existing
      const config = await readOpenclawConfig();
      const agentsSection = (config.agents && typeof config.agents === 'object' && !Array.isArray(config.agents))
        ? { ...(config.agents as Record<string, unknown>) }
        : {};
      const existingList = Array.isArray(agentsSection.list)
        ? [...(agentsSection.list as Array<Record<string, unknown>>)]
        : [];
      const existing = existingList.find((a: Record<string, unknown>) => a.workspace === `~/.openclaw/workspace-${slug}`);
      const existingAgentId = existing ? String(existing.id) : slug;
      return { workspacePath: newWorkspace, agentId: existingAgentId };
    }
    // Empty or broken — remove and recreate
...
export async function hireTeamFromMarketplaceTemplate(
  ...
  // NOTE: Only write openclaw-valid fields to openclaw.json. Custom metadata
  // (source, templateId, templateName, avatar) must NOT be persisted here —
  // avatar can be injected in-memory by listAgentsSnapshot() from template files.
