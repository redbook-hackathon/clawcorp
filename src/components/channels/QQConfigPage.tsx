/**
 * QQConfigPage — simplified config for QQ Bot channel
...
export interface QQConfigPageProps {
  onBack: () => void;
}

export function QQConfigPage({ onBack }: QQConfigPageProps) {
  const meta = CHANNEL_META.qqbot;
    ...
    (async () => {
      try {
        const result = await hostApiFetch<{ values?: Record<string, string> }>(
          '/api/channels/config/qqbot?accountId=default',
        );
  ...
  const handleValidate = async () => {
    ...
    try {
      const result = await hostApiFetch<{ valid: boolean; message?: string }>(
        ...
        {
          method: 'POST',
          body: JSON.stringify({ channelType: 'qqbot', config }),
        },
      );
      setValidationResult({
  ...
  return (
    <div className="flex h-full flex-col bg-white">
      ...
      <div className="flex h-[56px] shrink-0 items-center gap-3 border-b border-black/[0.06] px-5">
        ...
        <div>
          <h1 className="text-[15px] font-semibold text-[#111827]">QQ接入配置</h1>
          <p className="text-[12px] text-[#8e8e93]">填写QQ机器人配置信息</p>
        </div>
      </div>
