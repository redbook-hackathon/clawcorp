/**
 * WeComConfigPage — simplified config for WeCom channel
...
export interface WeComConfigPageProps {
  onBack: () => void;
}

export function WeComConfigPage({ onBack }: WeComConfigPageProps) {
  const meta = CHANNEL_META.wecom;
  ...
  const handleValidate = async () => {
    ...
    try {
      const result = await hostApiFetch<{ valid: boolean; message?: string }>(
        ...
        {
          method: 'POST',
          body: JSON.stringify({ channelType: 'wecom', config }),
        },
      );
      setValidationResult({
  ...
  return (
    <div className="flex h-full flex-col bg-white">
      ...
      <div className="flex h-[56px] shrink-0 items-center gap-3 border-b border-black/[0.06] px-5">
        ...
        <div>
          <h1 className="text-[15px] font-semibold text-[#111827]">企微接入配置</h1>
          <p className="text-[12px] text-[#8e8e93]">填写企业微信机器人配置信息</p>
        </div>
      </div>
